...
build_ldc_cmake_ninja: clone_master
	...
	cd master/build; cmake -G Ninja -DLLVM_CONFIG=/usr/local/bin/llvm-config50 BUILD_SHARED_LIB=ON BUILD_SHARED_LIBS=ON D_COMPILER=$(BOOTSTRAP_DMD) ..
	touch $@

build_ldc_cmake_make: clone_master
	[ -d master/build ] || mkdir master/build
	export DMD=$(BOOTSTRAP_DMD)
	cd master/build; cmake -DLLVM_CONFIG=/usr/local/bin/llvm-config50 BUILD_SHARED_LIB=ON BUILD_SHARED_LIBS=ON D_COMPILER=$(BOOTSTRAP_DMD) .. 
	touch $@
